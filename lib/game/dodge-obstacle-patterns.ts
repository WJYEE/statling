 ...
 * 2-lane spawn that always leaves exactly one lane open.
 */
export function pickDefaultSpawn(opts: { canDouble: boolean; lastLane: DodgeLane | null; rng?: () => number }): DodgeLane[] {
  const rng = opts.rng ?? Math.random
  const allLanes: DodgeLane[] = [0, 1, 2]
  if (opts.canDouble && rng() < 0.3) {
